...
const Index = () => {
  ...
  const handleEditItem = (index) => {
    if (!isDeleting) {
      ...
      const newItems = [...items];
      newItems[index].text = editValue; // update new data
      setItems(newItems);
      localStorage.setItem("items", JSON.stringify(newItems));
      setEditingIndex(null); //back two img edit and delete
      setIsEditing(true); // didn't work
    }
  };
  ...
  const completedTask = () => items.filter((item) => !item.showTick);
      ...
      <div className={styles.form}>
        <div className={styles.title}>
          <div className={styles.title_form}>
            {completedTask().length} task left
          </div>
